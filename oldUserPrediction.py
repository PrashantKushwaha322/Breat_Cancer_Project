...
def showold():
    ...
    rows = df.loc[global_1.a123]
    
    rowRadius=float(rows.radius_worst)
    rowtext=float(rows.texture_worst)
    rowperi=float(rows.perimeter_worst)
    rowarea=float(rows.area_worst )
    rowsmooth=float(rows.smoothness_worst)
    rowcompact=float(rows.compactness_worst)
    rowconca=float(rows.concavity_worst)
    rowconcav=float(rows.concave_points_worst)
    rowsymm=float(rows.symmetry_worst)
    rowfract=float(rows.fractal_dimension_worst)
    rowconcavi=float(rows.concavity_se)
    rowareas=float(rows.area_se)
    rowconcave=float(rows.concave_points_mean)
    rowtexture=float(rows.texture_mean)
    ...
    def save():
       ...
       # filename = 'test_122.csv'
        ##tempfile = NamedTemporaryFile(mode='w', delete=False)

        #fields = ['id','radius_worst','texture_worst','perimeter_worst','area_worst','smoothness_worst','compactness_worst','concavity_worst','concave_points_worst','symmetry_worst','fractal_dimension_worst','concavity_se','area_se','concave_points_mean','texture_mean','rowdiagonse']

        #with open(filename, 'r', encoding='utf8') as csvfile:#, tempfile:
         #   reader = csv.DictReader(csvfile, fieldnames=fields)
          #  writer = csv.DictWriter(tempfile, fieldnames=fields)
           # for row in reader:
            #  if row['id'] == str(global_1.a123):
             #   print('updating row', row['id'])
              #      row['id'], row['radius_worst'], row['texture_worst'],row['perimeter_worst'],row['area_worst'],row['smoothness_worst'],row['compactness_worst'],row['concavity_worst'],row['concave_points_worst'],row['symmetry_worst'],row['fractal_dimension_worst'],row['concavity_se'],row['area_se'],row['concave_points_mean'],row['texture_mean'] = rowRadius,rowtext, rowperi,rowarea,rowsmooth,rowcompact,rowconca,rowconcav,rowsymm,rowfract,rowconcavi,rowareas,rowconcave,rowtexture
               # row = {'id': row['id'],'radius_worst': row['radius_worst'],'texture_worst': row['texture_worst'],'perimeter_worst': row['perimeter_worst'],'area_worst': row['area_worst'],'smoothness_worst': row['smoothness_worst'],'compactness_worst': row['compactness_worst'],'concavity_worst': row['concavity_worst'],'concave_points_worst': row['concave_points_worst'],'symmetry_worst': row['symmetry_worst'],'fractal_dimension_worst': row['fractal_dimension_worst'],'concavity_se': row['concavity_se'],'area_se': row['area_se'],'concave_points_mean': row['concave_points_mean'],'texture_mean': row['texture_mean']}
                #writer.writerow(row)
        #shutil.move(tempfile.name, filename)
        
         rowRadius_1=float(e1.get())
         rowtext_1=float(e2.get())
         rowperi_1=float(e3.get())
         rowarea_1=float(e4.get())
         rowsmooth_1=float(e5.get())
         rowcompact_1=float(e6.get())
         rowconca_1=float(e7.get())
         rowconcav_1=float(e8.get())
         rowsymm_1=float(e9.get())
         rowfract_1=float(e10.get())
         rowconcavi_1=float(e11.get())
         rowareas_1=float(e12.get())
         rowconcave_1=float(e13.get())
         rowtexture_1=float(e14.get())
         ...
         f.close()
         result_1="Data is update successfully"
         messagebox.showinfo("Dataupdate",result_1)
    ...
    update = Button(root, text="update", fg="Black",
                    bg="Red", command=save)
    update.grid(row=17, column=1)
